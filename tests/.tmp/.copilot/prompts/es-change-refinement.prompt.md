...
Delegate to **es-ux-specialist agent**:
> "The change `<name>` is being refined. Adjustment: `<adjustment>`.
>
...
After update, delegate to **es-document-reviewer agent** to review.

### 5c. Update `architecture.md` (if affected and exists)

Delegate to **es-architect agent**:
> "The change `<name>` is being refined. Adjustment: `<adjustment>`.
>
...
If the change is in Phase B, C, or D (code has been written), delegate to **es-developer agent** to implement the code changes required by the adjustment:
...
If the change is in Phase B, C, or D, after the developer confirms implementation is complete, delegate to **es-tester agent** to update the tests:
...
Delegate to **es-tester agent** to create it:
...
Delegate to **es-tester agent** to update it:
