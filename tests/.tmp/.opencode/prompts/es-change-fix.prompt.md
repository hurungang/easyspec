...
```markdown
### Code Changes
**Modified by:** es-developer agent
...
If the fix revealed that acceptance criteria were unclear or incorrect, delegate to **es-product-owner agent**:
...
If updated, delegate to **es-document-reviewer agent** to review.

### 9b. Check if Spec Change needs updates

If the fix changed the intended behavior described in spec-change.md, delegate to **es-product-owner agent**:
...
- **Always run document-reviewer** after any doc updates
